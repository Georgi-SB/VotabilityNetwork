...
class Config(object):
    def __init__(self):
        self.L1_flag = True
        self.hidden_size = 100 #size of embeddings
        self.nbatches = 100
        self.nb_entities = 0
        self.nb_relations = 0
...
class TransEModel(object):
    def __init__(self, config):
        ...
        nb_entities = config.nb_entities
        nb_relations = config.nb_relations
        ...
        with tf.name_scope("embedding"):
            self.ent_embeddings = tf.get_variable(name="ent_embedding", shape=[nb_entities, size],
                                                  initializer=tf.contrib.layers.xavier_initializer(uniform=False))
            self.rel_embeddings = tf.get_variable(name="rel_embedding", shape=[relation_total, size],
                                                  initializer=tf.contrib.layers.xavier_initializer(uniform=False))
            #select the embeddings for the list of positions: pos_e, ...
